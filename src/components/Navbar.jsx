...
const Navbar = () => {
    ...
    return (
        <div className='flex justify-between items-center bg-white w-full h-[10vh] lg:h-[16vh] lg:px-12 px-6 fixed'>
            <img src={logo} alt="Logo" className='lg:w-[80px] w-[60px] h-auto items-center'/>
            <ul className={`${ulClass} ${visibilityClass}`}>
                ...
                <Link to='/Downloads'><li className='hover:scale-90 transition-all ml-auto'><button className='font-poppins2 border-2 rounded-2xl lg:px-8 lg:py-2 px-3 py-2 hover:scale-90 transition-all'>Download</button></li></Link>
            </ul>
            <img src={list} alt="Menu" className='lg:w-[20px] h-auto w-[16px] relative z-20 cursor-pointer icon' onClick={toggleList}/>
        </div>
    );
}
